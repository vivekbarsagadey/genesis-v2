...
import {
	Card, FormControl, IconButton, Modal, Paper, Button, Tooltip,
} from '@mui/material';
...
function CustomerKanbanView({ customer }: ListComponentProps) {
	...
	const newCustomers = customer.filter((ele: ICustomer) => ele.status == statusSet[0]);
	const activeCustomers = customer.filter((ele: ICustomer) => ele.status == statusSet[1]);
	const inActiveCustomers = customer.filter((ele: ICustomer) => ele.status == statusSet[2]);
