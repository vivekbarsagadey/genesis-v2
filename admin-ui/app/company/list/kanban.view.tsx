...
function CompanyKanbanView({ companies }: ListComponentProps) {
	...
	const newCompanies = companies.filter((ele: ICompany) => ele.status == statusSet[0],
		//  NEW,
	);
	const activeCompanies = companies.filter((ele: ICompany) => ele.status == statusSet[1],
		// ACTIVE,
	);
	const inActiveCompanies = companies.filter((ele: ICompany) => ele.status == statusSet[2],
		// INACTIVE
	);
