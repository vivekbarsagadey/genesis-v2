...
interface RoleFilterComponentProps {
  roles: Array<IRole>;
  onFilterHandler: (_: Array<IRole>) => void;
}
type FilterFields = {
  key: string;
  values: string[];
...
function RolesFilterComponent({
	roles,
	onFilterHandler,
}: RoleFilterComponentProps) {
	...
	const emailFilter = (value: string) => (item: IRole) => {
	...
	const nameFilter = (value: string) => (item: IRole) => {
		if (isNotBlank(item.name)) {
			if (!item.name.toLowerCase().includes(value.toLowerCase())) {
				return false;
			}
	...
	const applyFilter = () => {
		...
		// const d = roles.filter(() => emailFilter(emailData[0]));
	...
	useEffect(() => {
		...
		filterFields.push({ key: 'description', values: [], label: 'Description' });
