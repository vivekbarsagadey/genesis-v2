...
const RoleKanbanView = ({ roles }: ListComponentProps) => {
  const statusSet = Object.keys(Status).filter((v) => isNaN(Number(v)));

  const newRoles = roles.filter((ele: IRole) => {
    return ele.status == statusSet[0];
  });
  const activeRole = roles.filter((ele: IRole) => {
    return ele.status == statusSet[1];
  });
  const inActiveRole = roles.filter((ele: IRole) => {
            ...
            <Paper variant="outlined" className={classes.cardView}>
              <CardContent>
                ...
                {newRoles.reverse()?.map((newRole, index) => {
                  return <NewRolesComponent newRole={newRole} key={index} />;
                })}
              </CardContent>
            </Paper>
          </CardStyle>
...
const NewRolesComponent = ({ newRole }: INewRole) => {
  return (
    <Box mt={1}>
...
const ActiveRoleComponent = ({ activeRole }: IActiveRole) => {
  return (
    <Box mt={1}>
      <Card variant="outlined">
        <Grid container>
...
const InActiveRoleComponent = ({ inActiveRole }: IInActiveRole) => {
  return (
    <Box mt={1}>
      <Card variant="outlined">
        <Grid container>
