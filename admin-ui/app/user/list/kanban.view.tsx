...
import { Card, FormControl, IconButton, Modal, Paper, Select,Button,Tooltip } from '@mui/material';
        ...
        <Paper variant="outlined" className={classes.cardView}>
          <CardContent>
            ...
            {inActiveUsers.reverse()?.map((inActiveUser, index) => {
              return (
                <InActiveUserComponent inActiveUser={inActiveUser} key={index} />
              );
            })}
          </CardContent>
        </Paper>
...
const NewUserComponent = ({ newUser }: INewUser) => {
    ...
    <Card variant="outlined" style={{borderRadius:'6px'}}>
      <Grid container>
        <Grid item xs={6} display="flex" alignItems="center" pl={2} pb={1}>
          ...
          {`${newUser.firstName} ${newUser.lastName} `}          </Typography>
        </Grid>
        <Grid
          item
          xs={6}
        ...
        <Card variant="outlined">
          <Grid container>
            <Grid item xs={6} display="flex" alignItems="center" pl={2} pb={1}>
              <PersonIcon fontSize="inherit" />
              <Typography noWrap pl={1} variant="h6">
              {`${inActiveUser.firstName} ${inActiveUser.lastName} `}              </Typography>
            </Grid>
            <Grid
              item
              xs={6}
